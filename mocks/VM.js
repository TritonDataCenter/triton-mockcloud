...
function createMockVMDir(uuid, opts)
{
    var dir = path.join('/mockcn', process.env.MOCKCN_SERVER_UUID, 'vms');
    var split = dir.substr(1).split('/');
    var tmp = '';

    split.forEach(function (_dir) {
        tmp = tmp + '/' + _dir;
        try {
            if (opts.log) {
                opts.log.info({dir: tmp}, 'trying to create dir');
            }
            fs.mkdirSync(tmp);
        } catch (e) {
            if (opts.log) {
                opts.log.error({err: e, dir: tmp}, 'failed to create dir');
            }
            if (e.code !== 'EEXIST') {
                throw (e);
            }
...
function createMockVM(payload, opts, callback)
{
    var filename;
    var uuid = payload.uuid;

    filename = getMockVMJSONFilename(uuid);
    createMockVMDir(uuid, opts);

    // fixup payload for a VM instead of a create
    if (payload.hasOwnProperty('add_nics')) {
        payload.nics = payload.add_nics;
        delete payload.add_nics;
    }

    // TODO: we should set to provisioning, wait some amount of time then switch
    //       to 'installed' or 'running'. Or failed? We should also emit events via
    //       vminfod in these cases.
    if (payload.autoboot) {
        payload.state = 'running';
        payload.zone_state = 'running';
    } else {
        payload.state = 'stopped';
        payload.zone_state = 'installed';
    }

    if (opts.log) {
        opts.log.info({filename: filename, payload: payload}, 'trying to create file for VM');
    }
...
function createVM(payload, log, callback)
{
    ...
    throw new Error('UNIMPLEMENTED: createVM');
...
function createZone(payload, log, callback)
{
    ...
    createMockVM(payload, {log: log}, function (err) {
        callback();
    });
}
